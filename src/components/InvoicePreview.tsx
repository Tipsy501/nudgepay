...
export default function InvoicePreview({ data, isSmall = false }: InvoicePreviewProps) {
  ...
  return (
    <div className={containerClasses}>
      {/* Header */}
      <div className={`flex justify-between items-start ${isSmall ? 'mb-10' : 'mb-16'}`}>
        <div className="space-y-6">
          <div className="space-y-1">
            <h2 className={`font-black tracking-tighter text-slate-900 ${isSmall ? 'text-2xl' : 'text-3xl'}`}>
      ...
      <div className={`mt-16 pt-10 border-t border-slate-50 flex flex-col md:flex-row justify-between items-start gap-12 ${isSmall ? 'mt-8 pt-6' : ''}`}>
        <div className="flex-1 max-w-sm">
          <p className="text-[9px] font-black text-slate-300 uppercase tracking-[0.2em] mb-3">Terms & Acknowledgements</p>
          <p className={`text-slate-400 font-medium leading-relaxed italic ${isSmall ? 'text-[10px]' : 'text-[10px]'}`}>
        ...
        <div className={`w-full ${isSmall ? 'md:w-56' : 'md:w-72'}`}>
          ...
          <div className={`p-6 bg-slate-900 rounded-2xl text-white flex justify-between items-center shadow-[var(--shadow-hex)] ${isSmall ? 'p-5' : ''}`}>
            <span className="text-[9px] font-black uppercase tracking-[0.2em] opacity-40">Final Total Due</span>
            <span className={`font-black tracking-tighter ${isSmall ? 'text-xl' : 'text-3xl'}`}>
